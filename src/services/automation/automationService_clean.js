...
const automationService = {
  ...
  async _runProductionMode(
    ...
    try {
      ...
      // Create environment with Chrome profile
      const env = { ...process.env };
      env.LINKEDIN_CHROME_PROFILE_PATH = this.getChromeProfilePath();
  ...
  _setupProcessHandlers(
    ...
    pythonProcess.on("close", (code) => {
      ...
      if (code === 0) {
        ...
        const errorDetails = {
          message: `LinkedIn automation failed with exit code ${code}`,
          exitCode: code,
          stdout: stdoutData,
          stderr: stderrData,
          usedBundled,
  ...
  async checkChromeAvailability() {
    ...
    return new Promise((resolve) => {
      ...
      if (process.platform === "win32") {
        ...
        // Check if chrome.exe is in PATH first
